    ...
    <form method="POST" action="{{ route('admin.categories.update', $category) }}">
        ...
        <div class="mb-4">
            ...
            <select name="section" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-amber-400">
                <option value="Sundown Healthy" {{ old('section', $category->section) == 'Sundown Healthy' ? 'selected' : '' }}>Sundown Healthy</option>
                <option value="Crazy Corner" {{ old('section', $category->section) == 'Crazy Corner' ? 'selected' : '' }}>Crazy Corner</option>
            </select>
